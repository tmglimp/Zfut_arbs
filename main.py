...
def file_has_valid_header(path):
    """
    Check if the CSV file at 'path' contains expected headers. We expect to see at least one of these keys: "serverId", "symbol", or "conid".
    """
    try:
...
if __name__ == "__main__":
    ...
    if not statuses.get("FUTURES.index", False):
        ...
        try:
            ...
            config.FUTURES = pd.DataFrame(df)
