...
def format_value(value, A_symbol, B_symbol, A_incr, B_incr):
    ...
    print(f"Exact price with Futures contract-specific incremental rounding is being printed as {final_price}")
    return final_price
...
def check_and_cancel_orders():
    ...
    try:
        ...
        print("Could not load placed_orders_runtime.csv. Assuming it's empty.")
    ...
    try:
        ...
        print(f"Error removing filled orders from placed_orders: {e}")

    required_columns = {"status", "remainingQuantity", "orderId"}
    if df_orders.empty or not required_columns.issubset(df_orders.columns):
        print("Warning: No valid orders returned or required fields missing from API response.")
    ...
    if df_pending.empty:
        if _last_empty_time is None:
            ...
            config.NO_PENDING_ORDERS = True
            print("\nNo pending orders for more than 5 minutes — setting config.NO_PENDING_ORDERS = True")
    else:
        _last_empty_time = None
        config.NO_PENDING_ORDERS = False

    if df_placed.empty:
        print("placed_orders.csv is empty. Cancelling all pending orders.")
    ...
    if matched_pending.shape[0] > 4:
        print(f"\nThere are {len(matched_pending)} matched pending orders. Cancelling oldest to maintain max of 5...")
        ...
        try:
            ...
            print(f"Error cancelling order {oldest_order_id}: {e}")
...
def orderRequest(updated_ORDERS: pd.DataFrame | None = None) -> None:
    ...
    value = first_row["TOTAL_BASIS"]*-1

    value_after_fees = value - calculate_total_fees(
        "CBOT",
        first_row["A_MARKETSYMBOL"],
        "CBOT",
        first_row["B_MARKETSYMBOL"],
    )
    ...
    if reduced_value <= 0:
        ...
        config.updated_ORDERS = config.updated_ORDERS
        return
    ...
    exact_price = format_value(
        ...
        first_row["A_MARKETSYMBOL"],
        first_row["B_MARKETSYMBOL"],
        first_row["A_MINPRICEINCREMENT"],
        first_row["B_MINPRICEINCREMENT"],
    )
    print("exact_price as", exact_price)

    url = f"{config.IBKR_BASE_URL}/v1/api/iserver/account/{config.IBKR_ACCT_ID}/orders"

    front_conid = int(first_row["A_CONID"])
    back_conid = int(first_row["B_CONID"])
    ...
    if order_req.status_code != 200:
        print("Failed to place order. Skipping this attempt.")
        return
    ...
    try:
        ...
        print("Error parsing order response:", exc)
        return
