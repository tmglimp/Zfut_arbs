...
def sp_cal(ahdgscombos_basis, ahdgscombos_vol, bhdgscombos_basis, bhdgscombos_vol):
    return (ahdgscombos_basis * ahdgscombos_vol)-(bhdgscombos_basis * bhdgscombos_vol)

def rv_cal(ahdgscombos_basis, ahdgscombos_vol, bhdgscombos_basis, bhdgscombos_vol):
    return (bhdgscombos_basis * bhdgscombos_vol)-(ahdgscombos_basis * ahdgscombos_vol)
...
def net_basis(gross, carry):
    return (gross+ carry)

def vol_wghtd_basis(zlnvol, pairs_net):
    return (zlnvol*pairs_net)
...
def calculate_quantities_with_sma(HEDGES_Combos):
    ...
    for leg in ['A', 'B']:
        coupons = HEDGES_Combos[f'{leg}_CTD_COUPON']
        mat_dates = pd.to_datetime(HEDGES_Combos[f'{leg}_CTD_MATURITY_DATE'])
        fut_prices = HEDGES_Combos[f'{leg}_PRICE']
        cfs = HEDGES_Combos[f'{leg}_CTD_CONVERSION_FACTOR']
        HEDGES_Combos[f'{leg}_AccruedInterest'] = [
            accrued_interest(c, m, current_date) for c, m in zip(coupons, mat_dates)
        ]
        HEDGES_Combos[f'{leg}_DirtyPrice'] = HEDGES_Combos[f'{leg}_CTD_BPRICE']
...
def calculate_quantities(HEDGES_Combos, SMA):
    ...
    HEDGES_Combos["A_FRST_LTR_INT"] = (HEDGES_Combos["A_PRODUCTDESCRIPTION"].astype(str).str[0]
        .apply(lambda x: ord(x.upper()) if x.isalpha() else int(x) if x.isdigit() else ""))
    HEDGES_Combos["B_FRST_LTR_INT"] = (HEDGES_Combos["B_PRODUCTDESCRIPTION"].astype(str).str[0]
        .apply(lambda x: ord(x.upper()) if x.isalpha() else int(x) if x.isdigit() else ""))
    HEDGES_Combos = HEDGES_Combos[
        (HEDGES_Combos["A_FRST_LTR_INT"] < HEDGES_Combos["B_FRST_LTR_INT"]) &
        (abs(HEDGES_Combos['A_YTM'] - HEDGES_Combos['B_YTM']) >= 0.1) &
        (HEDGES_Combos['B_YTM'] - abs(HEDGES_Combos['A_YTM']) <= (96 / 360))
        ]

    # Convert volume columns to numeric.
    HEDGES_Combos['A_FUT_VOLUME'] = pd.to_numeric(HEDGES_Combos['A_SIZE'], errors='coerce')
    HEDGES_Combos['B_FUT_VOLUME'] = pd.to_numeric(HEDGES_Combos['B_SIZE'], errors='coerce')
    ...
    HEDGES_Combos['ANetBasis'] = HEDGES_Combos.apply(
        lambda row: net_basis(gross=row['A_GrossBasis'],carry=row['A_Carry']), axis=1)
    HEDGES_Combos['BNetBasis'] = HEDGES_Combos.apply(
        lambda row: net_basis(gross=row['B_GrossBasis'],carry=row['B_Carry']), axis=1)

    HEDGES_Combos['sp_cal'] = HEDGES_Combos.apply(
        lambda row: sp_cal(ahdgscombos_basis=row['ANetBasis'],ahdgscombos_vol=row['Base_Ln_A'],bhdgscombos_basis=row['BNetBasis'],bhdgscombos_vol=row['Base_Ln_B'] ), axis=1)
    HEDGES_Combos['rv_cal'] = HEDGES_Combos.apply(
        lambda row: rv_cal(ahdgscombos_basis=row['ANetBasis'],ahdgscombos_vol=row['Base_Ln_A'],bhdgscombos_basis=row['BNetBasis'],bhdgscombos_vol=row['Base_Ln_B'] ), axis=1)
    HEDGES_Combos = HEDGES_Combos.sort_values(by='sp_cal', ascending=True)

    HEDGES_Combos.to_csv('HEDGES_Combos.csv')

    unique_rows = (HEDGES_Combos
        .drop_duplicates(subset=['A_CONID', 'B_CONID'], keep='first')
        .copy())
